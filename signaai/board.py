...
Quick start:
    ...
    from signaai.board import open_task, list_tasks, claim_task, accept_claim

    task_id, tx = open_task(passphrase, task_hash, "research", amount_signa=1.0,
                            board_address="S-BOARD-ADDR-HERE")
    tasks = list_tasks(board_address="S-BOARD-ADDR-HERE")

Environment:
    SIGNAAI_BOARD — default board address (avoids passing board_address everywhere)
"""
import hashlib
import os
import sys
...
def _resolve_board(board_address):
    addr = board_address or os.environ.get("SIGNAAI_BOARD", "")
    if not addr:
        raise ValueError(
...
def _task_id(payer_address, task_hash):
    """Deterministic unique task ID: short hash of payer + task_hash + nanosecond timestamp."""
...
def cancel_task(passphrase, task_id, board_address=None, network=None):
    ...
    If an escrow was already created, cancel it separately via escrow module.
...
def get_claims(task_id, board_address=None, limit=50, network=None):
    """
    Return all TASK:CLAIM events for a given task_id.
...
def get_board_events(board_address=None, limit=100, network=None):
    """
    Return all TASK: events on the board (OPEN, CLAIM, ACCEPT, CANCEL).

    Useful for building an indexer or monitoring tool.
    """
    board = _resolve_board(board_address)
